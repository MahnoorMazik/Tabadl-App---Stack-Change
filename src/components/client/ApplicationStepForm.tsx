...
type StepFieldRowProps = {
  ...
  locale: string
  uploading: boolean
  onChange: (fieldId: string, value: string, opts?: { render?: boolean }) => void
  onFileUpload: (fieldId: string, file: File | undefined) => void
  onRemoveFile: (fieldId: string) => void
}

const StepFieldRow = memo(function StepFieldRow({
  field,
  index,
  value,
  fileName,
  ...
  // Local state for free-text typing — avoids re-rendering the whole form every keystroke.
  ...
  return (
    ...
    >
      ...
      {field.type === 'TEXTAREA' ? (
        <Textarea
          ...
          onChange={(e) => {
            ...
            setLocalValue(next)
            onChange(field.fieldId, next, { render: false })
          }}
          ...
          className={cn(
            isRTL ? 'text-right' : 'text-left',
            engaging
              ? 'disabled:opacity-80 disabled:cursor-not-allowed focus-visible:ring-emerald-500 disabled:bg-gray-100'
              : undefined
          )}
        />
        ...
        <Input
          ...
          className={cn(
            isRTL ? 'text-right' : 'text-left',
            engaging
              ? 'focus-visible:ring-emerald-500 h-10 disabled:opacity-80 disabled:cursor-not-allowed disabled:bg-gray-100'
              : undefined
          )}
        />
      )}
      ...
      {error ? (
        <p className={cn('text-xs text-destructive', isRTL ? 'text-right' : 'text-left')}>{error}</p>
      ) : null}
    </div>
  )
})
...
export function ApplicationStepForm({
  ...
  const localizedFormName = useMemo(() => getLocalizedText(formName, locale), [formName, locale])
  ...
  const latestValuesRefHolder = useRef(latestValuesRef)
  latestValuesRefHolder.current = latestValuesRef
  ...
  const syncValues = useCallback((next: Record<string, string>, fromServer = false) => {
    valuesRef.current = next
    const latest = latestValuesRefHolder.current
  ...
  useEffect(() => {
    ...
    setErrors({})
  }, [fieldsSyncKey, serverSyncVersion, syncValues])

  const setValue = useCallback((fieldId: string, value: string, opts?: { render?: boolean }) => {
    const next = { ...valuesRef.current, [fieldId]: value }
    valuesRef.current = next
    const latest = latestValuesRefHolder.current
    if (latest) latest.current = next
    // Text/textarea pass render:false so typing stays local to the field row.
  ...
  const setFileName = useCallback((fieldId: string, name: string) => {
  ...
  const handleFileUpload = useCallback(
    async (fieldId: string, file: File | undefined) => {
      if (!file) return
    ...
    [applicationId, fileUploadBasePath, setFileName, setValue]
  )
    ...
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <Card
        className={cn(
          ...
          engaging ? 'shadow-sm' : 'shadow-none'
        )}
      >
        <CardHeader
          className={cn(
        ...
        >
          ...
          >
            <div
              className={cn(
                'space-y-1 min-w-0',
                isRTL ? 'text-right order-last' : 'text-left order-first'
              )}
            >
            ...
            <div
              className={cn(
                'flex items-center gap-2 flex-wrap shrink-0 min-h-7',
                isRTL ? 'flex-row order-first' : 'flex-row order-last'
              )}
            >
              {headerActions}
              {saveIndicator === 'saving' && (
                <span
                  className={cn(
                    'inline-flex items-center text-xs text-muted-foreground bg-background/80 border rounded-full px-2.5 py-1 gap-1',
                    isRTL && 'flex-row-reverse'
                  )}
                >
              ...
              {saveIndicator === 'saved' && (
                <span
                  className={cn(
                    'inline-flex items-center text-xs text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-full px-2.5 py-1 gap-1',
                    isRTL && 'flex-row-reverse'
                  )}
                >
          ...
          <div className={cn('space-y-3', engaging && 'space-y-4')}>
            {fields.map((field, idx) => (
              <StepFieldRow
                ...
                isRTL={isRTL}
                locale={locale}
                uploading={uploadingFieldId === field.fieldId}
                onChange={setValue}
