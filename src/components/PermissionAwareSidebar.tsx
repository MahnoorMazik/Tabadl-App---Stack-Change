...
export function PermissionAwareSidebar({
  ...
  const componentId = useRef(
    `sidebar-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
  ).current;
  ...
  const getFilteredSidebarItems = useCallback((): SidebarItem[] => {
    ...
    const filtered = ALL_SIDEBAR_ITEMS.map((item) => {
      ...
      if (
        item.permission &&
        !permissionChecker.hasPermission(item.permission as any)
      )
        return null;
      ...
      const filteredChildren = item.children?.filter((child) => {
        if (
          child.permission &&
          !permissionChecker.hasPermission(child.permission as any)
        )
          return false;
        return true;
      });
              ...
              >
                {item.children!.map((child, index) => (
                  <div
                    ...
                    className={`transition-all duration-200 ease-in-out ${isOpen ? "translate-y-0" : "-translate-y-2"
                      }`}
                    style={{
    ...
    >
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        {filteredItems.map((item, index) => (
          <div key={`${item.title}-${index}`}>{renderSidebarItem(item)}</div>
        ))}
      </nav>
    </div>
  );
